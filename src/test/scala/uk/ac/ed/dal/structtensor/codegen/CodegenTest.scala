...
class CodegenTest extends AnyFlatSpec with Matchers {
  ...
  it should "generate code for when there is an optimal projection, like matrix covariance matrix without the body" in {
    Utils.cnt = 0
    Main.main(
      Array(
        "-i",
        "examples/covar.stur",
        "-o",
        "src/test/resources/test_outputs/covar_wo_body_test.cpp"
      )
    )

    val file1 = scala.io.Source.fromFile(
      "src/test/resources/correct_test_outputs/covar_wo_body.cpp"
    )
    val file2 = scala.io.Source.fromFile(
      "src/test/resources/test_outputs/covar_wo_body_test.cpp"
    )
  ...
  it should "generate code for when there is an optimal projection, like pseudo lr training without the body" in {
    Utils.cnt = 0
    Main.main(
      Array(
        "-i",
        ...
        "-o",
        "src/test/resources/test_outputs/pseudo-lr-training_wo_body_test.cpp"
      )
    )

    val file1 = scala.io.Source.fromFile(
      "src/test/resources/correct_test_outputs/pseudo-lr-training_wo_body.cpp"
    )
    val file2 = scala.io.Source.fromFile(
      "src/test/resources/test_outputs/pseudo-lr-training_wo_body_test.cpp"
    )
