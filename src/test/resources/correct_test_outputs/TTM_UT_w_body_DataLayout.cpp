...
int flag159 = 0 <= i && M > i && 0 <= j && N > j && 0 <= l && Q > l && i <= j;
if (flag159) {
...
double **B2 = new double*[((M * (N + 1)) / 2)];
for (size_t s = 0; s < ((M * (N + 1)) / 2); ++s) {
B2[s] = new double[Q];
...
cerr << A[0][0][0] << endl;
cerr << C[0][0] << endl;
for (size_t i0 = 0; i0 < Q; ++i0) {delete[] B2[i0];
}delete[] B2;
