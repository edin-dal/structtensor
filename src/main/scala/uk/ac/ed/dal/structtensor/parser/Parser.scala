...
object Parser {
  ...
  def parseComparison(op: String, i1: Index, i2: Index): Seq[Comparison] = (i1, i2) match {
  ...
  def access[$: P]: P[Access] = P( name.! ~ "(" ~ variableSeq ~ ")" ).map({case (n, v) => parseAccess(n, v)}) | P("(" ~ access ~ ")") 
  ...
  def remainder[$: P]: P[Index] = P(index ~ (remainder_op ~ index).rep).map({case (i, sOpI) => parseArith(i, sOpI)})

  def mult_div[$: P]: P[Index] = P(remainder ~ (mult_div_op ~ remainder).rep).map({case (i, sOpI) => parseArith(i, sOpI)})

  def arithmetic[$: P]: P[Index] = P(mult_div ~ (add_sub_op ~ mult_div).rep).map({case (i, sOpI) => parseArith(i, sOpI)})
    
  def index[$: P]: P[Index] = P( variable ) | P( integer ) | P( decimal ) | P( "(" ~ arithmetic ~ ")" )

  def comp_op[$: P]: P[String] = P( "<=" | ">=" | "==" | "=" | "<" | ">" | "!=" ).!

  def comparison[$: P]: P[Seq[Comparison]] = P (arithmetic ~ comp_op ~ arithmetic).map({case (i1, op, i2) => parseComparison(op, i1, i2)})
    
  def exp[$: P]: P[Seq[Exp]] = P(access.rep(exactly=1)) | P(comparison) | P( "(" ~ exp ~ ")" )

  def factor[$: P]: P[Prod] = P( exp ~ ("*" ~ exp).rep ).map{case(e, se) => parseProd(e, se)} | P( "(" ~ factor ~ ")")
