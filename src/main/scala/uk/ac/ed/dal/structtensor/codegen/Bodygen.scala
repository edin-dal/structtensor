...
object Bodygen {
  ...
  def generateInit(codeLang: String, rules: Seq[Rule], all_tensors: Seq[Access], all_dimensions: Map[Access, Seq[Dim]], uniqueSets: Map[Access, Rule], initTensors: Boolean, symbols: Seq[Variable], outputs_names: Seq[String]): String = {
    initTensors match {
      case true => {
        ...
        val only_lhs_heads = rules.map(_.head).filter(_.kind == Tensor).distinctBy(_.name)
