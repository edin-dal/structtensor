...
object Compiler {
  ...
  def shiftDimInfer(lhs: Access, rhs: Access, exps: Seq[Exp]): Rule =
    Rule(lhs.dimensionHead, SoP(Seq(Prod(Seq(rhs.dimensionHead) ++ exps))))
  ...
  def binMultDimInfer(lhs: Access, e1: Exp, e2: Exp): Rule = {
    (e1, e2) match {
      case (
        ...
        Rule(
          lhs.dimensionHead,
          SoP(Seq(Prod(Seq(acc1.dimensionHead, acc2.dimensionHead))))
        )
      case (
            acc1 @ Access(name1, vars1, Tensor),
            Comparison(op2, index2, variable2)
          ) =>
        Rule(lhs.dimensionHead, SoP(Seq(Prod(Seq(acc1.dimensionHead, e2)))))
      case (Comparison(op1, index1, variable1), Access(name2, vars2, Tensor)) =>
        binMultDimInfer(lhs, e2, e1)
      case (
            Comparison(op1, index1, variable1),
            Comparison(op2, index2, variable2)
          ) =>
        Rule(lhs.dimensionHead, SoP(Seq(Prod(Seq(e1, e2)))))
      case _ =>
        throw new Exception(
          "Expression is either a comparison or an access. This error should never happen. Function: binMult"
        )
  ...
  def binAddDimInfer(lhs: Access, e1: Exp, e2: Exp): Rule = {
    (e1, e2) match {
      case (acc1 @ Access(_, vars1, _), acc2 @ Access(_, vars2, _))
        ...
        Rule(
          lhs.dimensionHead,
          SoP(Seq(Prod(Seq(acc1.dimensionHead)), Prod(Seq(acc2.dimensionHead))))
        )
      case _ =>
        throw new Exception(
          "Input expressions to binary addition must be accesses with equal set of variables."
