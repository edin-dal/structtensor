...
"""
    ...
    val c3 = other_const_tensorComputation.foldLeft("")((acc, ctc) => s"$acc\n${codeGen(ctc, other_const_dimInfo, other_const_uniqueSets, other_const_redundancyMap, 1, false, compressionMaps=other_const_compressionMap, codeMotion=codeMotion, sturOpt=sturOpt)}")
...
"""
    ...
    val c9 = other_const_tensorComputation.foldLeft("")((acc, ctc) => s"$acc\n${codeGen(ctc, other_const_dimInfo, other_const_uniqueSets, other_const_redundancyMap, 1, sparse, codeLang="MLIR", sturOpt=sturOpt)}")

    val c10 = MLIR_start_timer_code()
    val c11 = peq_tensorComputation.foldLeft("")((acc, ctc) => s"$acc\n${codeGen(ctc, peq_dimInfo, peq_uniqueSets, peq_redundancyMap, 1, sparse, codeLang="MLIR", sturOpt=sturOpt)}")
    val cerr_load = s"""
