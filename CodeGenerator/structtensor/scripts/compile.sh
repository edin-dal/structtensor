...
do
  echo Compiling ${name}_cm.cpp
  clang -lstdc++ ${name}_cm.cpp -std=c++17 -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_cm
  echo Compiling ${name}_Sparse_cm.cpp
  clang -lstdc++ ${name}_Sparse_cm.cpp -std=c++17 -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_Sparse_cm
  echo Compiling ${name}_no_cm.cpp
  clang -lstdc++ ${name}_no_cm.cpp -std=c++17 -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_no_cm
  echo Compiling ${name}_Sparse_no_cm.cpp
  clang -lstdc++ ${name}_Sparse_no_cm.cpp -std=c++17 -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_Sparse_no_cm
done

cd ../outputs_c
for name in ${structtensor_names[@]}
do
  echo Compiling ${name}_cm.c
  clang ${name}_cm.c -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_cm
  echo Compiling ${name}_Sparse_cm.c
  clang ${name}_Sparse_cm.c -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_Sparse_cm
  echo Compiling ${name}_no_cm.c
  clang ${name}_no_cm.c -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_no_cm
  echo Compiling ${name}_Sparse_no_cm.c
  clang ${name}_Sparse_no_cm.c -O3 -ffast-math -march=native -ftree-vectorize -Rpass=loop-vectorize -o ${name}_Sparse_no_cm
done
# cd ../outputs_mlir
