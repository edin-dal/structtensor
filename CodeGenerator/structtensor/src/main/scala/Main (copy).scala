...
//   def infer(tensorComputation: Rule, dimInfo: Seq[DimInfo], uniqueSets: Map[Exp, Rule], redundancyMaps: Map[Exp, Rule], mode: Int = 0): (Rule, Rule) = {
//     val mode: Int = 0
...
//         }}
//         val multInfers: Map[Access, (Rule, Rule)] = similarNameMap.foldLeft(Map.empty[Access, (Rule, Rule)])((acc, cur) => {
...
//         val newE: Seq[Exp] = exps.zipWithIndex.filter(eI => !similarNameIndices.contains(eI._2)).map(eI => eI._1) ++ multInfers.keySet.toSeq
//         val similarUSMap: Map[Exp, Rule] = multInfers.map{case (access, us_rm) => (access -> us_rm._1)}
//         val similarRMMap: Map[Exp, Rule] = multInfers.map{case (access, us_rm) => (access -> us_rm._2)}
...
//         val noCompArithExps: Seq[(Exp, Int)] = newE.zipWithIndex.filter(eI => {
//           val e = eI._1
//           if (e.isInstanceOf[Comparison]) {
//             val comp: Comparison = e.asInstanceOf[Comparison]
//             if (comp.op == "=" && comp.index.isInstanceOf[Arithmetic]) {
//               val arith: Arithmetic = comp.index.asInstanceOf[Arithmetic]
//               if (arith.op == "-" && arith.index1.isInstanceOf[Variable] && arith.index2.isInstanceOf[Variable]) false
//               else true
//             } else true
//           } else true
//         })
//         println("newE", newE)
//         println("noCompArithExps", noCompArithExps)

//         val newExps: Seq[Exp] = if(mode == 0) newE else noCompArithExps.map{case (e, i) => e}
//         val remainingExps: Seq[Exp] = if(mode == 0) Seq.empty[Exp] else (0 to newExps.length - 1).filter(i => !noCompArithExps.map(eI => eI._2).contains(i)).map(i => newE(i))

//         if (newExps.length == 1) return (multInfers.values.toSeq)(0)
//         else if (newExps.length == 2) {
//           val sop: SoP = SoP(Seq(Prod(Seq(newExps(0), newExps(1)) ++ remainingExps)))
//           val finalTC: Rule = Rule(head, sop)
//           return infer(finalTC, dimInfo, uniqueSetsSim, redundancyMapsSim, mode)
//         }
...
//         val startHead: Access = Access(getVar("head"), getVariables(Prod(Seq(newExps(0)))) ++ getVariables(Prod(Seq(newExps(1)))), Tensor)
//         val startSoP: SoP = SoP(Seq(Prod(Seq(newExps(0), newExps(1)))))
//         val startTC: Rule = Rule(startHead, startSoP)
//         val inf: (Rule, Rule) = infer(startTC, dimInfo, uniqueSetsSim, redundancyMapsSim, mode)
...
//         println("2")
//         val (tc, inliningMap, (newUniqueSet, newRedundancyMap)) = seqInd.foldLeft((startTC, tcMap, (newUS, newRM)))((acc, i) => {
//           val accTC: Rule = acc._1
//           val tcMap: Map[Access, SoP] = acc._2
//           val us: Map[Exp, Rule] = acc._3._1
//           val rm: Map[Exp, Rule] = acc._3._2
//           val e: Exp = newExps(i)
//           val head: Access = Access(getVar("head"), accTC.head.vars ++ getVariables(Prod(Seq(e))), Tensor)
//           val sop: SoP = SoP(Seq(Prod(Seq(accTC.head, e))))
//           val tc: Rule = Rule(head, sop)
//           val inf: (Rule, Rule) = infer(tc, dimInfo, us, rm, mode)
...
//           (tc, newTCMap, (newUS, newRM))
//         })
//         println("3")
//         val sop: SoP = SoP(Seq(Prod(Seq(tc.head, newExps(n)) ++ remainingExps)))
//         val finalTC: Rule = Rule(head, sop)
//         println("mode:", mode)
//         println(finalTC.prettyFormat)
//         return infer(finalTC, dimInfo, newUniqueSet, newRedundancyMap, mode)
//       }
//     } else {
//       if (prods.length == 2) {
...
//       // else
//       val (tcSeq, inliningMap, (newUniqueSet, newRedundancyMap)) = prods.foldLeft((Seq.empty[Rule], Map.empty[Access, SoP], (uniqueSets, redundancyMaps)))((acc, prod) => {
//         val accTCSeq: Seq[Rule] = acc._1
//         val tcMap: Map[Access, SoP] = acc._2
//         val us: Map[Exp, Rule] = acc._3._1
//         val rm: Map[Exp, Rule] = acc._3._2
//         val allVars: Seq[Variable] = getVariables(prod)// prod.exps.foldLeft(Seq.empty[Variable])((acc, e) => acc ++ getAllVariables(e)).distinct
...
//         val newRM: Map[Exp, Rule] = mergeMap(Seq(rm, Map[Exp, Rule](head -> inf._2)))((v1, v2) => v2)
//         val newTCMap: Map[Access, SoP] = mergeMap(Seq(tcMap, Map(head -> sop)))((v1, v2) => concatSoP(Seq(v1, v2)))
//         (accTCSeq :+ tc, newTCMap, (newUS, newRM))
//       })
//       println("tcSeq", tcSeq)
//       val n = prods.length - 1
//       val seqInd: Seq[Int] = 2 to n - 1
//       val startHead: Access = Access(getVar("head"), (tcSeq(0).head.vars ++ tcSeq(1).head.vars).distinct, Tensor)
//       println("isDirectSum", isDirectSum(tcSeq(0).head, ))
//       val startSoP: SoP = SoP(Seq(Prod(Seq(tcSeq(0).head)), Prod(Seq(tcSeq(1).head))))
//       val startTC: Rule = Rule(startHead, startSoP)
//       val inf: (Rule, Rule) = infer(startTC, dimInfo, newUniqueSet, newRedundancyMap, mode)
...
//       val (tc, finalInliningMap, (finalUniqueSet, finalRedundancyMap)) = seqInd.foldLeft((startTC, tcMap, (newUS, newRM)))((acc, i) => {
//         val accTC: Rule = acc._1
//         val tcMap: Map[Access, SoP] = acc._2
//         val us: Map[Exp, Rule] = acc._3._1
//         val rm: Map[Exp, Rule] = acc._3._2
//         val e: Access = tcSeq(i).head
//         val head: Access = Access(getVar("head"), (accTC.head.vars ++ e.vars).distinct, Tensor)
...
//       val finalTC: Rule = Rule(head, sop)
//       val finalInf: (Rule, Rule) = infer(finalTC, dimInfo, finalUniqueSet, finalRedundancyMap, mode)
//       return finalInf
//     }
...
//   def codeGen(tensorComputation: Rule, dimInfo: Seq[DimInfo], uniqueSets: Map[Exp, Rule], redundancyMaps: Map[Exp, Rule], mode: Int = 0): String = {
//     val variables: Seq[Variable] = getVariables(tensorComputation)
//     val (outUS, outRM) = infer(tensorComputation, dimInfo, uniqueSets, redundancyMaps, mode)
...
//   def mttkrp_n(structure: String): (Rule, (Rule, Rule)) = {
//     val head: Access = Access("A", Seq("i".toVar, "j".toVar), Tensor)
//     val var1: Access = Access("B",  Seq("i".toVar, "k".toVar, "l".toVar), Tensor)
//     val var2: Access = Access("C",  Seq("k".toVar, "j".toVar), Tensor)
//     val var3: Access = Access("D",  Seq("l".toVar, "j".toVar), Tensor)
...
//     val inf: (Rule, Rule) = infer(tensorComputation, dimInfo, newUS, newRM, 1)
...
//     println(codeGen(mult_tensorComputation, mult_dimInfo, mult_uniqueSets, mult_redundancyMap, 1))
//     println("===========================================================")
//   }
