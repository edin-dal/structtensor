...
"builtin.module"() ({
  ...
  "func.func"() ({
    ^computebb(%A: memref<?x?xf64>, %B: memref<?x?x?xf64>, %C: memref<?x?xf64>, %D: memref<?x?xf64>, %P1 : index, %Q1: index, %M1: index, %N1: index, %I1: index):
    %zerof = "arith.constant"() {"value" = 0.0 : f64} : () -> f64
    // %c0 = arith.constant 0 : index
    // %p = memref.dim %D, %c0 : memref<?x?xf64>
    %i = affine.apply affine_map<(i) -> (i)> (%I1)
    // "affine.if"(%i, %M1) ({

    affine.for %j = 0 to %Q1 step 1 {

    %k_sum = affine.for %k = 0 to %N1 step 1 iter_args(%iter_sum_k = %zerof) -> (f64) {

    %l_sum = affine.for %l = 0 to %P1 step 1 iter_args(%iter_sum_l = %zerof) -> (f64) {
    ...
    %prod3 = "arith.mulf"(%tmp0, %tmp1) {"fastmath" = #arith.fastmath<fast,ninf,reassoc,nnan,nsz,arcp,contract,afn>} : (f64, f64) -> f64
    %finalV28 = "arith.mulf"(%prod3, %tmp2) {"fastmath" = #arith.fastmath<fast,ninf,reassoc,nnan,nsz,arcp,contract,afn>} : (f64, f64) -> f64


    %new_sum_l = "arith.addf"(%finalV28, %iter_sum_l) {"fastmath" = #arith.fastmath<fast,ninf,reassoc,nnan,nsz,arcp,contract,afn>} : (f64, f64) -> f64
    "affine.yield"(%new_sum_l): (f64) -> ()
    }
    %new_sum_k = "arith.addf"(%l_sum, %iter_sum_k) {"fastmath" = #arith.fastmath<fast,ninf,reassoc,nnan,nsz,arcp,contract,afn>} : (f64, f64) -> f64
    "affine.yield"(%new_sum_k): (f64) -> ()
    }
    %preVal30 = affine.load %A[%i, %j] : memref<?x?xf64>
    // %preVal30 = "memref.load"(%A, %i, %j) : (memref<?x?xf64>, index, index) -> f64
    // %preVal30 = "affine.load"(%A, %i, %j) {map = affine_map<(d0, d1) -> (d0, d1)>} : (memref<?x?xf64>, index, index) -> f64
    %peqVal29 = "arith.addf"(%k_sum, %preVal30) {"fastmath" = #arith.fastmath<fast,ninf,reassoc,nnan,nsz,arcp,contract,afn>} : (f64, f64) -> f64
    affine.store %peqVal29, %A[%i, %j] : memref<?x?xf64>
    // "memref.store"(%peqVal29, %A, %i, %j) : (f64, memref<?x?xf64>, index, index) -> ()
    // "affine.store"(%peqVal29, %A, %i, %j) {map = affine_map<(d0, d1) -> (d0, d1)>} : (f64, memref<?x?xf64>, index, index) -> ()
    "affine.yield"() : () -> ()
    }
    // "affine.yield"() : () -> ()
    // }, {"affine.yield"(): () -> ()}) {condition = affine_set<(d0)[s1]: (d0 <= s1 - 1)>} : (index,index) -> ()
    func.return
  }) {function_type = (memref<?x?xf64>, memref<?x?x?xf64>, memref<?x?xf64>, memref<?x?xf64>, index, index, index, index, index) -> (), sym_name = "compute", sym_visibility = "private"} : () -> ()
  ...
  "func.func"() ({
    ...
    "func.call"(%A, %B, %C, %D, %P, %Q, %M, %N, %I) {callee = @compute}: (memref<?x?xf64>, memref<?x?x?xf64>, memref<?x?xf64>, memref<?x?xf64>, index, index, index, index, index) -> ()
