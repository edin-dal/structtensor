...
"builtin.module"() ({
  ...
  "func.func"() ({
    ...
    %Ji32 = "func.call"(%argv4) {callee = @atoi} : (!llvm.ptr<i8>) -> i32
    ...
    %gen_cond1 = "arith.cmpi"(%i3, %J) {predicate = 0 : i64} : (index, index) -> i1   
...
affine.if affine_set<()[N, J] : (N >= J + 1)> () [%N, %J] {
  affine.for %i = affine_map<()[] -> (0)> () [] to affine_map<()[M] -> (M)> () [%M] step 1 {
    affine.for %j = affine_map<()[] -> (0)> () [] to affine_map<()[P] -> (P)> () [%P] step 1 {
      %A_0 = affine.load %C[%i, symbol(%J), %j] : memref<?x?x?xf64>
      %A_1 = affine.load %B[%i, symbol(%J), %j] : memref<?x?x?xf64>
      %A_2 = arith.mulf %A_0, %A_1 : f64
      affine.store %A_2, %A[%i, symbol(%J), %j] : memref<?x?x?xf64>
    }
  }
}
